...
| `updateAreaGeometry(areaId, draft)` | `updatePolygonGeometry(polygonId, draft)` | ルートポリゴンのみ |
...
| `loadAreaToDraft(areaId)` | `loadPolygonToDraft(polygonId)` | ルートポリゴンのみ |
...
| `NotRootPolygonError` | ルートポリゴン限定の操作を非ルートに対して呼んだ |
